...
describe('AuditConsumer', () => {
  ...
  it('should consume and save audit entry to database', async () => {
    ...
    expect(assertQueueWithDLQ).toHaveBeenCalledWith(QUEUES.AUDIT_LOGS, { durable: true });
    expect(prisma.auditTrail.create).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should retry on database failure and eventually nack to DLQ', async () => {
    ...
    const consumerPromise = startAuditConsumer();
